...
### 2.23
  `nullptr`可以转换成任意其他的指针类型。之后判断`p`是否指向合法对象，只要把`p`作为`if`语句的条件就可以了。如果p的值是`nullptr`，则条件为假；反之为真。

---
---
### 2.24
  `void*` 是一种特殊的指针类型，可以用于存放任意对象的地址，`p`是合法的；
  
  `lp`是不合法的，因为`lp`是一个长整型指针，而i只是一个普通整型数，二者的类型不匹配。
  
---
---

### 2.25
 - (a)类型：`ip`是整型指针 值：整型数在内存中的地址；类型：`i`是一个整型数；类型：`r`是一个引用，绑定`i` 值：`r==i`；
  
 - (b)类型：`i`是一个整型数；`ip`是一个整型指针； 值：`ip`的值为0
 
 - (c)类型：`ip`是一个整型指针；`ip2`是一个整型数 值：所指整型数在内存中的地址；
